<img src="https://raw.githubusercontent.com/antfu/vue-i18n-ally/master/static/logo.png" alt="logo" width="150" align="right"/>

# Vue i18n Ally

🌍 更好的 [Vue i18n](https://github.com/kazupon/vue-i18n) 翻译体验 - [**English Version**](https://github.com/antfu/vue-i18n-ally/blob/master/README.md)

[![Visual Studio Marketplace Version](https://img.shields.io/visual-studio-marketplace/v/antfu.vue-i18n-ally.svg?color=blue&label=VSCode%20Marketplace&logo=data:image/svg+xml;base64,)](https://marketplace.visualstudio.com/items?itemName=antfu.vue-i18n-ally)
[![Visual Studio Marketplace Downloads](https://img.shields.io/visual-studio-marketplace/d/antfu.vue-i18n-ally.svg)](https://marketplace.visualstudio.com/items?itemName=antfu.vue-i18n-ally)
[![Visual Studio Marketplace Installs](https://img.shields.io/visual-studio-marketplace/i/antfu.vue-i18n-ally.svg)](https://marketplace.visualstudio.com/items?itemName=antfu.vue-i18n-ally)
[![GitHub last commit](https://img.shields.io/github/last-commit/antfu/vue-i18n-ally.svg)](https://github.com/antfu/vue-i18n-ally)
[![GitHub issues](https://img.shields.io/github/issues/antfu/vue-i18n-ally.svg)](https://github.com/antfu/vue-i18n-ally/issues)

## ⚡ 主要功能

- 内联翻译显示
- 自动补全
- 一键机器翻译
- 统一管理所有翻译文案
- 翻译进度报告
- 从代码中提取文案
- 转跳到翻译文件
- 支持`JSON`和`YAML`
- 支持多目录工作区
- 支持 [`vue-i18n`](https://github.com/kazupon/vue-i18n), [`vuex-i18n`](https://github.com/dkfbasel/vuex-i18n), [`vue-i18next`](https://github.com/panter/vue-i18next) 和 [`nuxt-i18n`](https://github.com/nuxt-community/nuxt-i18n)
- 插件自身多语言支持 **(English, 简体中文, 繁體中文)**

![](https://raw.githubusercontent.com/antfu/vue-i18n-ally/master/screenshots/overview-zh-cn.png)


## ⚙ 安装

从 VSCode插件市场 [安装插件](https://marketplace.visualstudio.com/items?itemName=antfu.vue-i18n-ally) 即可!


## 🎯 常见问题

### 插件没有作用

本插件只会在装有 `vue-i18n` 的项目中启用。请确保在 `package.json` 的 `dependencies` 或 `devDependencies` 中装有下列任一依赖。
  - [`vue-i18n`](https://github.com/kazupon/vue-i18n)
  - [`vuex-i18n`](https://github.com/dkfbasel/vuex-i18n)
  - [`vue-i18next`](https://github.com/panter/vue-i18next)
  - [`nuxt-i18n`](https://github.com/nuxt-community/nuxt-i18n)

### 我可以看到插件的图标显示在侧边栏，但没有任何的翻译文案

可能的问题：

1. **区域设置路径配置缺失**，您可能需要通过下列任一方式手动配置翻译文件路径
     - 打开**命令面板**（`Ctrl-Shift-P`或`⌘⇧P`），选择 `Vue i18n 助手: 手动设置语言目录` 并按照指南操作。
     - 打开 VSCode 的设置文件并手动设置 `vue-i18n-ally.localesPath`。
2. **源语言/显示语言设置**。 默认语言环境设置为英语（`en`）。 如果您的项目不支持英语，则可能需要通过命令 `Vue i18n 助手: 更改源语言` 进行修改。
3. **目录结构** 请阅读下一节


## 📂 目录结构

您可以将语言目录置为一下两种形式

```
  locales
  ├── en.json
  ├── de-DE.json
  ├── zh-CN.yaml  # YAML 格式同样支持
  ├── zh-TW.yaml
  ├── ...
  └── <contry-code>.json
```

or

```
  locales
  ├── en
  |   ├── common.json
  |   ├── buttons.json
  |   ├── ...
  |   └── <filenames>.json
  ├── de-DE
  |   ├── common.json
  |   ├── buttons.json
  |   └── ...
  └── <contry-code>
      ├── common.json
      ├── buttons.json
      └── ...
```

目前我们支持`json`和`yaml`两种翻译文件格式。

如果您想使用不同的文件格式或目录结构，欢迎提 Issue 或 PR。

## 👨‍💻 感谢

本插件最初Fork自 [think2011/vscode-vue-i18n](https://github.com/think2011/vscode-vue-i18n)，非常感谢 [@think2011](https://github.com/think2011) 的支持和启发。

## License

[MIT License](https://github.com/antfu/vue-i18n-ally/blob/master/LICENSE) © 2019 [Anthony Fu](https://github.com/antfu)

MIT License © 2018-2019 [think2011](https://github.com/think2011)
